...
Tocab = loaded_model.Tocab
Secab = loaded_model.Secab
Index2Vocab = loaded_model.Index2Vocab
Index2Tocab = loaded_model.Index2Tocab
    ...
    for j = 1, batchSize do
        ...
        for j2=1,#sentBatch_text[j]do   
            if  Vocab[sentBatch_text[j][j2]] ~= nil then 
                ...
                if Tocab[sentBatch_tags[j][j2]] ~= nil then 
                    encoderTags[   j2 + eosOffset ][j] = Tocab[sentBatch_tags[j][j2]]
                else
                    encoderTags[   j2 + eosOffset ][j] = Tocab['<unknown>']
                end
            else
                encoderInputs[ j2 + eosOffset ][j] = 0 --Vocab['<unknown>']
                encoderTags  [ j2 + eosOffset ][j] = 0 --Tocab['<unknown>']
            end
        end
...
eval = function()
    ...
    for i = 1,numDocuments/batchSize do
        ...
        for batchSize_ind = 1,batchSize do
            str_sent = "" 
            enc_batch_size = Inputs[1][1]:select(2,batchSize_ind)
    ...
    evalUserSmy:close()
    evalPredSmy:close()
    io.open('target_summary.txt', 'a'):close()
...
<<<<<<< HEAD
    ...
    elseif opt.testfile == 'cvValid' then 
        file_txt = io.open('./cv_text_words_valid.csv', 'r')
        file_tag = io.open('./cv_text_words_tags_valid.csv', 'r')
        file_tag = io.open('./cv_summary_words_valid.csv', 'r')
    elseif opt.testfile == 'cvTest' then 
        file_txt = io.open('./cv_text_words_test.csv', 'r')
        file_tag = io.open('./cv_text_words_tags_test.css', 'r')
        file_tag = io.open('./cv_summary_words_test.csv', 'r')
    else
        print("INVALID OPTION --testfile ['test', 'cvValid', 'cvTest']" .. opt.testfile)
        os.exit()
    end
=======
    file_txt = io.open('test_text_words.csv','r')
    file_txt = io.open('test_text_words_tags.csv','r')
    file_smy = io.open('test_summary_words.csv','r')
>>>>>>> parent of d1025b3... Temporary
