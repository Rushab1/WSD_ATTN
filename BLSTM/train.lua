...
cmd = torch.CmdLine()
    ...
    cmd:option('-LSTM_type', 'bidirectional') --bidirectional, unidirectional_forward, unidirectional_backward
opt = cmd:parse(arg)
...
lookup = nn.LookupTableMaskZero(VocabSize, hiddenSize)
if(opt.glove == 'True') then
    f = torch.load(opt.glove_file)

    for i = 3, #Index2Vocab do
        if f[Index2Vocab[i]] == nil then 
            lookup.weight[i] = lookup.weight[1]
        else
            lookup.weight[i] = f[Index2Vocab[i]]
        end
    end
end
