...
function Seq2Seq:__init(vocabSize, secabSize, hiddenSize)
  self.vocabSize = assert(vocabSize, "vocabSize required at arg #1")
  self.secabSize = assert(secabSize, "SenseVocabSize required at arg #1")
...
function Seq2Seq:buildModel()
  ...
  self.encoder:add(nn.Sequencer(nn.LookupTableMaskZero(self.vocabSize, self.hiddenSize)))
  --self.encoderLSTM = nn.FastLSTM(self.hiddenSize, self.hiddenSize):maskZero(1)
  self.encoderLSTM = nn.FastLSTM(self.hiddenSize, self.secabSize):maskZero(1)
  --self.encoder:add(nn.BiSequencer(self.encoderLSTM))
  self.encoder:add(nn.Sequencer(self.encoderLSTM))
  --self.encoder:add(nn.SelectTable(1))
  --self.encoder:add(nn.Select(1,-1))

  --self.encoder:add(nn.Sequencer(nn.MaskZero(nn.Linear(2*self.hiddenSize, self.secabSize),1)))
  --self.encoder:add(nn.Sequencer(nn.MaskZero(nn.LogSoftMax(),1)))
  --self.encoder:add(nn.ArgMax())

  self.decoder = nn.Sequential()
  --self.decoderLookup = (nn.Sequencer(nn.LookupTableMaskZero(self.vocabSize, self.hiddenSize)))

  --self.decoder:add(self.decoderLookup)
  --self.decoder:add(nn.Sequencer(nn.LookupTableMaskZero(self.vocabSize, self.hiddenSize)))
...
function Seq2Seq:forwardConnect()
  self.decoderLSTM.userPrevOutput =
    nn.rnn.recursiveCopy(self.decoderLSTM.userPrevOutput, self.encoderLSTM.outputs)
  self.decoderLSTM.userPrevCell =
    nn.rnn.recursiveCopy(self.decoderLSTM.userPrevCell, self.encoderLSTM.cells)
end
...
function Seq2Seq:eval(input,checkword,Secab,Index2Secab,SecabSize)
  ...
  prediction = self.encoder:forward(input)

  _prediction = prediction[1]:select(1,(#prediction[1])[1])
  prob, wordIds = _prediction:topk((#_prediction)[1], 1, true, true)
